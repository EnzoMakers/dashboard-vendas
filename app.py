...
def presentation_mode():
    ...
    with col1:
        if st.button("🏠 Voltar ao Dashboard"):
            st.session_state.just_returned_from_presentation = True
            st.session_state.filter_key_suffix = str(time.time())  # Force widget recreation
            st.session_state.page_mode = "dashboard"
            st.rerun()

    
    st.markdown("---")
    st.write("Conteúdo da apresentação será desenvolvido aqui...")
...
# Se estiver no modo apresentação, redirecionar
if st.session_state.page_mode == "presentation":
    # Aqui vai a função da página de apresentação
    presentation_mode()
    st.stop()
...
if uploaded_files:
    ...
    default_index = 0
    if 'selected_file_name' in st.session_state and st.session_state.selected_file_name in file_names:
    ...
    try:
        ...
        if 'tp_mov' in filtered_df.columns and not filtered_df['tp_mov'].empty:
            ...
            if just_returned and 'saved_tp_mov' in st.session_state:
                default_tp_mov = [x for x in st.session_state.saved_tp_mov if x in all_tp_mov]
        ...
        if 'representante' in filtered_df.columns and not filtered_df['representante'].empty:
            ...
            if just_returned and 'saved_representantes' in st.session_state:
                default_representantes = [x for x in st.session_state.saved_representantes if x in all_representantes]
        ...
        if 'cliente' in filtered_df.columns and not filtered_df['cliente'].empty:
            ...
            if just_returned and 'saved_clientes' in st.session_state:
                default_clientes = [x for x in st.session_state.saved_clientes if x in all_clientes]
        ...
        if 'descricao' in filtered_df.columns and not filtered_df['descricao'].empty:
            ...
            if just_returned and 'saved_produtos' in st.session_state:
                default_produtos = [x for x in st.session_state.saved_produtos if x in all_produtos]
